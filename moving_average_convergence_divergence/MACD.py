...
def calc_best_emas(df, short_ema, maxema, steps):
    ...
    for long_ema in df.columns[5:]:
        ...
        if short_ema_index < long_ema_index:
            ...
            return_string = indicator_name + ', ' + str(indicator_result)
...
def main():
    ...
    if(maxema == 100 and steps == 10):
        print('[i] Default values used for maxema and steps..')
    ...
    with Bar('[+] Calculating best EMAs...') as bar:
        progress = (1/len(df.columns[5:])) * 100
        for short_ema in df.columns[5:]:
            process = pool.apply_async(calc_best_emas, args=(df, short_ema, maxema, steps,), callback=results.append)
            process.wait()
            bar.next(progress)
    ...
    outfile.write('Indicator, Result\n')
